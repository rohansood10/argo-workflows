...
func writeTracingGo(filename string, spans *spansList, attributes *attributesList) error {
	...
	defer f.Close()
	tracingHeaderTmpl.Execute(f, map[string]string{"Banner": generatedBanner, "Filename": filepath.Base(filename)})
	...
	for _, span := range *spans {
		if hasOptionalAttributes(&span.Attributes) {
			...
			generateSpanOption(f, &span.common, optionTypeName, attributes)
		}
	}
	...
	for _, span := range *spans {
		...
		spanTmpl.Execute(f, map[string]string{
		...
		fmt.Fprintf(f, "}\n\n")
		generateStart(f, &span, attributes)
		//		generateEnd(f, &span)
	}
...
func generateStart(f io.Writer, s *span, attributes *attributesList) {
	...
	spanStartTmpl.Execute(f, map[string]any{
	...
	if len(requiredAttrs) > 0 || hasOptionalAttributes(allowed) {
		...
		if len(requiredAttrs) > 0 {
			...
			for _, allowedAttr := range requiredAttrs {
				...
				fmt.Fprintf(&b, "\t\t%s(\"%s\", %s),\n", attr.kvConstructor(), allowedAttr.Name, paramName)
			}
			b.WriteString("\t}\n")
		} else {
			b.WriteString("\n")
...
func generateSpanOption(f io.Writer, c *common, optionTypeName string, attributes *attributesList) {
	// Generate the option type
	spanOptionTmpl.Execute(f, map[string]string{
	...
	for _, attr := range c.Attributes {
		if attr.Optional {
			...
			spanOptionFuncTmpl.Execute(f, map[string]string{
				"FuncName":        funcName,
