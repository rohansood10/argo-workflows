...
func writeMetricsHelpersGo(filename string, metrics *metricsList, attributes *attributesList) error {
	...
	// Write file header
	metricHeaderTmpl.Execute(f, map[string]string{"Banner": generatedBanner, "Filename": filename})
	...
	for _, metric := range *metrics {
		if hasOptionalAttributes(&metric.Attributes) {
			...
			generateMetricOption(f, &metric.common, optionTypeName, attributes)
		}
	}
...
func generateMetricOption(f io.Writer, c *common, optionTypeName string, attributes *attributesList) {
	// Generate the option type
	metricOptionTmpl.Execute(f, map[string]string{
	...
	for _, attr := range c.Attributes {
		if attr.Optional {
			...
			metricOptionFuncTmpl.Execute(f, map[string]string{
				"FuncName":       funcName,
