...
// ClearSpans is a no-op - tests should filter spans by trace ID
func (c *OTELCollector) ClearSpans() error {
	return nil
}
...
// GetSpansAfter returns all spans that started after the given time
func (c *OTELCollector) GetSpansAfter(ctx context.Context, after time.Time) ([]CollectedSpan, error) {
	spans, err := c.GetSpans(ctx)
	if err != nil {
		return nil, err
	}
