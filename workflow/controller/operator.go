...
func (woc *wfOperationCtx) markWorkflowPhase(ctx context.Context, phase wfv1.WorkflowPhase, message string) context.Context {
	...
	if woc.wf.Status.Phase != phase {
		...
		woc.log.WithFields(logging.Fields{"spanContext": trace.SpanFromContext(ctx).SpanContext()}).Info(ctx, "SpanContext on Phase Change")
		...
		woc.wf.Annotations[common.AnnotationKeyTraceID] = trace.SpanFromContext(ctx).SpanContext().TraceID().String()
