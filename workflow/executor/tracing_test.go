...
func TestTracingArtifactLoadSpans(t *testing.T) {
	...
	val, ok := initAttribs.Value("WorkflowName")
	require.True(t, ok)
	assert.Equal(t, fakeWorkflow, val.AsString())
	val, ok = initAttribs.Value("WorkflowNamespace")
	require.True(t, ok)
	...
	val, ok = loadArtifactAttribs.Value("ArtifactPath")
	require.True(t, ok)
	...
	val, ok = unarchiveAttribs.Value("ArtifactArchive")
	require.True(t, ok)
...
func TestTracingArtifactSaveSpans(t *testing.T) {
	...
	val, ok := waitAttribs.Value("WorkflowName")
	require.True(t, ok)
	assert.Equal(t, fakeWorkflow, val.AsString())
	val, ok = waitAttribs.Value("WorkflowNamespace")
	require.True(t, ok)
	assert.Equal(t, fakeNamespace, val.AsString())
